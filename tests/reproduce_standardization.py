...
def verify_standard_format(output_dir: Path, split: str = "train"):
    ...
    print("✅ Found facts.csv and targets.csv")
    ...
    if "hops" in targets_df.columns:
        ...
        if not inferred.empty:
            ...
            if max_hops == 0:
                print("⚠️  Warning: Inferred facts have 0 hops? This might be wrong.")
        else:
            print("ℹ️  No positive inferred facts found in this small sample.")
...
def run_ont_generator_verification():
    ...
    try:
        ...
        # Verify
        if (
            verify_standard_format(output_dir, "train")
            and verify_standard_format(output_dir, "val")
            and verify_standard_format(output_dir, "test")
        ):
...
def run_asp_generator_verification():
    ...
    cmd = [
        ...
        "asp_generator",
        "python",
        "apps/asp_generator/src/asp_generator/convert_to_csv.py",
        "dataset=family_tree",
        "train_val_pct=0.8",
        "val_pct=0.1",
    ]
    ...
    if (
        verify_standard_format(output_base, "train")
        and verify_standard_format(output_base, "val")
        and verify_standard_format(output_base, "test")
    ):
...
def run_rrn_loader_verification():
    ...
    try:
        ...
        class MockSchema:
            ...
            def get_relation_index(self, name):
                return 0
