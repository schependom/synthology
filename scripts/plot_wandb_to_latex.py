...
OWN_COLORS = {"geeloranje": "#f7aa25", "appelblauwzeegroen": "#0cc795"}
...
def main():
    ...
    parser.add_argument("--runs", nargs="+", required=True, help="List of WandB run IDs or Display Names to plot.")
    parser.add_argument(
        "--labels", nargs="*", help="List of legend labels corresponding to the runs. Must match --runs length."
    )
    parser.add_argument(
        "--colors",
        nargs="*",
    ...
    parser.add_argument(
        "--metric", type=str, help="Metric name (e.g., 'triple_pr_auc'). Use 'all' to bulk export all shared metrics."
    )
    parser.add_argument(
        "--list-metrics",
        action="store_true",
    ...
    parser.add_argument(
        "--output", type=str, default="plot.pdf", help="Output path. Ignored if '--metric all' is used."
    )
    parser.add_argument(
        "--smooth",
        type=float,
    ...
    parser.add_argument("--title", type=str, default="", help="Optional title for the plot.")
    parser.add_argument("--ylabel", type=str, help="Override Y-axis label. If not provided, uses section/metric.")
    parser.add_argument("--xlabel", type=str, default="Steps", help="Override X-axis label.")
    parser.add_argument(
        "--log-y-substrings",
        nargs="*",
        default=["loss"],
        help="List of substrings. If a metric name contains any of these, its y-axis will be logarithmic (e.g. 'loss').",
    )
    ...
    for i, run_id in enumerate(args.runs):
        ...
        run_data_list.append({"label": label, "color": color_hex, "df": df, "run_id": run_id})

    print("\nGenerating plots...")
    for metric in metrics_to_plot:
        metric_key = f"{args.section}/{metric}"

        # Check if this metric matches any of the logarithmic substrings
        use_log_scale = any(sub.lower() in metric.lower() for sub in args.log_y_substrings)
        ...
        # Apply logarithmic scale if a substring matched
        if use_log_scale:
            ax.set_yscale("log")
