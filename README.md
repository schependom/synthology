...
- **Method**:
    - Train RRN on 500 KGs generated by LUBM
...
- **Method**:
    - Train the RRN on increasingly small subsets of data
    - E.g. 50, 100, 500 KGs.
- **Metric**: Learning curves for each subset size
- **Expected Result**: An intelligent generator should reach high accuracy with *fewer* training samples because every sample is designed to be a "lesson" in logic.

### 4. Scalability

- **Goal**: Show that the generator can scale to large ontologies (way more complex than family tree)
- **Method**:
    - Generate ontologies of increasing complexity
- **Metric**:
    - Generation time
    - Memory usage
    - Training time
