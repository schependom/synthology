# Experiment 2: Multi-Hop Reasoning Quality

## Objective

Experiment 2 tests the paper's central structural claim for the Family Tree ontology: even after enforcing deep-path parity on the UDM baseline, data generated by Synthology yields better downstream multi-hop reasoning performance.

Paper traceability:

- Methodology framing: paper section "Experiment 2: Multi-Hop Reasoning Quality".
- Baseline fairness design: paper section "The Unguided Deductive Materialization (UDM) Baseline", especially the parity-enforcement protocol.
- Expected interpretation: results/discussion text around parity-enforced comparisons and deep-path structural quality.

## Datasets Used

- Ontology: `ontologies/family.ttl`.
- Exp2 Synthology train/val/test output root: `data/exp2/synthology/family_tree/`.
- Exp2 UDM baseline train/val/test output root: `data/exp2/baseline/family_tree/`.
- Frozen shared deep test set root: `data/exp2/frozen_test/`.

All task wrappers also archive canonical run artifacts under:

- `reports/experiment_runs/<date>/exp2/<task>/<timestamp>/`

## Experiment Mechanics (What Is Being Validated)

1. Generate a frozen deep test set once and reuse it for both models.
2. Generate baseline and Synthology training datasets under comparable budget constraints.
3. Enforce baseline parity with retry-loop generation until deep and structural criteria are satisfied.
4. Train one RRN on baseline and one RRN on Synthology with equivalent training protocol.
5. Compare model metrics and data diagnostics, with explicit deep-hop reporting.

Why iterative Jena is required in Exp2:

- Standard Jena closure is fixed-point materialization and does not preserve derivation depth metadata natively.
- Exp2 parity requires depth-aware evidence, so the baseline config uses iterative materialization (`materialization.iterative: true`) to recover hop-layer structure for parity diagnostics.

## Configurations (Source of Truth)

- Baseline generator: `configs/udm_baseline/exp2_baseline.yaml`
- Synthology generator: `configs/ont_generator/exp2_synthology.yaml`
- Frozen test generation: `configs/ont_generator/exp2_gold_test.yaml`
- RRN baseline training: `configs/rrn/exp2_baseline_hpc.yaml`
- RRN Synthology training: `configs/rrn/exp2_synthology_hpc.yaml`
- Exp2 comparison report: `configs/data_reporter/exp2_compare.yaml`

Important fixed semantics in current config/task stack:

- Exp2 baseline uses Jena profile `owl_mini`.
- Exp2 baseline iterative mode is enabled in `configs/udm_baseline/exp2_baseline.yaml`.
- Parity loop/report commands archive their outputs in run archives and keep attempt-level artifacts in the attempts root for reproducibility.

## Preconditions

Before running Exp2 baseline/parity on a fresh machine or HPC node:

```bash
uv sync
module load openjdk/21
./install-mvn.sh
which java && java -version
which mvn && mvn -v
```

Optional quick Java preflight:

```bash
cd apps/udm_baseline/java
mvn -q -DskipTests package
ls -lh target/jena-materializer-1.0.0-shaded.jar
cd ../../..
```

## Execution Commands (Canonical Order)

### 1. Freeze Exp2 deep test set

```bash
uv run invoke exp2-generate-gold-test
```

### 2. Generate baseline dataset (UDM + Jena iterative)

```bash
uv run invoke exp2-generate-baseline
```

### 3. Generate Synthology dataset

```bash
uv run invoke exp2-generate-synthology
```

### 4. Optional strict budget matching helper

Use this when you want both generators to share explicit train caps:

```bash
uv run invoke exp2-balance-datasets --fact-cap=<FACT_CAP> --target-cap=<TARGET_CAP>
```

### 5. Run parity loop (time-to-parity + structural parity)

```bash
uv run invoke exp2-parity-loop
```

### 6. Build parity summary report

```bash
uv run invoke exp2-parity-report
```

### 7. Build Exp2 distribution/comparison report

```bash
uv run invoke exp2-report-data
```

### 8. Train RRN on baseline and Synthology

```bash
uv run invoke exp2-train-rrn --dataset=baseline
uv run invoke exp2-train-rrn --dataset=synthology
```

### 9. Optional smoke and paper plots

```bash
uv run invoke exp2-smoke-jena-visual

uv run invoke paper-visual-report \
  --exp2-synth-targets=data/exp2/synthology/family_tree/train/targets.csv \
  --exp2-parity-summary=data/exp2/baseline/parity_runs/parity_loop_summary.json \
  --out-dir=reports/paper
```

## High-Value Overrides (When Reviewer Asks For Sensitivity)

Baseline generation caps:

```bash
uv run invoke exp2-generate-baseline --fact-cap=250000 --target-cap=200000 --base-facts-per-sample=18
```

Synthology generation caps:

```bash
uv run invoke exp2-generate-synthology --fact-cap=250000 --target-cap=200000 --proof-roots-per-rule=10
```

Parity strictness example:

```bash
uv run invoke exp2-parity-loop \
  --max-attempts=400 \
  --min-deep-hops=3 \
  --tolerance-pct=10.0 \
  --deep-count-mode=tolerance
```

Default note:

- The task wrapper default tolerance is 10% to favor convergence speed; tighten this value for stricter parity sensitivity checks.

## Expected Results (What Must Be Reported)

Data-level outputs to include:

- Deep-path count target and achieved parity traces (`d >= 3`).
- Time-to-parity metrics (baseline cumulative runtime vs Synthology generation runtime).
- Depth-bucket diagnostics (at minimum: total, hop=0, hop=1, hop=2-3, hop>=3).
- Structural parity indicators (nodes, edge density, inferred share, target ratios as configured).

Model-level outputs to include:

- `PR-AUC`, `AUC-ROC`, `F1`, and `FPR` for baseline-trained and Synthology-trained RRN.
- Same test split, same evaluation protocol, same frozen Exp2 deep test reference.

Primary logging locations:

- Run archives: `reports/experiment_runs/...`
- Dataset outputs: `data/exp2/...`
- Paper plots: `reports/paper/...`
- Model training runs: W&B + RRN run archives.

## Reviewer Checklist

Use this checklist before sharing results:

1. Confirm baseline config has `materialization.iterative: true` and `jena_profile: owl_mini`.
2. Confirm both RRN runs were executed after the same frozen test-set generation.
3. Confirm parity loop/report artifacts exist and are archived with manifests/logs.
4. Confirm reported metrics include both global classification metrics and hop-bucket diagnostics.
5. Confirm command history can be reconstructed from `reports/experiment_runs/.../manifest.json`.

## Cross-Reference

For the full paper-wide sequence (Exp1-Exp3 + final figures), use `experiments/PAPER_RUNBOOK.md`.
