...
class Generator(object):
    ...
    def _print_distribution(cls, counts: typing.Dict[str, int]):
        ...
        stats_pattern = "{{index:{}}} {{name:{}}}  {{count:{}}} {{bars:{}}}".format(
            index_len, label_len, num_len, cls.BAR_CHART_STEPS
        )
    ...
    def _print_stats(cls, pos_counts: typing.Dict[str, int], neg_counts: typing.Dict[str, int]):
        ...
        # create pattern for printing a single line
        stats_pattern = (
            "{{index:{}}} {{name:{}}}  {{num_pos:{}}} {{pos_bars:>{}}}{{neg_bars:{}}} {{num_neg:{}}}".format(
                index_len,
                label_len,
                pos_len,
    ...
    def _sample_family_tree(cls, conf: config.Config) -> typing.List[person.Person]:
        ...
        while True:
            ...
            can_add_children = len(current_person.children) < conf.max_branching_factor and (
            ...
            if add_child:
                ...
                if current_person.married_to:
                    ...
                    spouse = pf.PersonFactory.create_person(
                        current_person.tree_level + 1, female=not current_person.female
                    )
            ...
            if (
                p >= conf.max_tree_size
                or total_attempts >= conf.max_tree_size * 10
                or (conf.stop_prob > 0 and random.random() < conf.stop_prob)
            ):
                break
    ...
    @classmethod
    def _write_sample(
        cls, conf: config.Config, family_tree: typing.List[person.Person], data: aspwrapper.AnswerSet, base_name: str
    ) -> None:
            ...
            if len(f.terms) == 1:
                if f.predicate in cls.CLASSES:
                    individuals[f.terms[0]].classes.add(
                        class_membership.ClassMembership(classes[f.predicate], f.positive)
                    )
            ...
            if len(i.terms) == 1:
                if i.predicate in cls.CLASSES:
                    individuals[i.terms[0]].classes.add(
                        class_membership.ClassMembership(classes[i.predicate], i.positive, inferred=True)
                    )
    ...
    def generate(cls, conf: config.Config) -> None:
        ...
        tree_size_counts = {
            title_format.format(size): counts for size, counts in enumerate(tree_size_counts) if size > 1
        }
