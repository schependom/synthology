    ...
    def on_validation_epoch_start(self, trainer: pl.Trainer, pl_module: pl.LightningModule) -> None:
        ...
        for i, batch in enumerate(self._cached_batches):
            ...
            for t in all_triples:
                ...
                # Some rows can carry predicate index -1 (or out-of-range) depending on
                # dataset/schema alignment. Skip them for artifact logging to avoid calling
                # the class MLP with relation inputs.
                if pred_idx is None or pred_idx < 0 or (pred_idx + 1) >= len(pl_module.mlps):
                    continue
